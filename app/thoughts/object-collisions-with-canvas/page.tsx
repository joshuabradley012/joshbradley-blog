...
class Canvas {
  ...
  ...
        ...
        <p>
          ...
          outside of creating an elaborate matrix to represent coordinates. And
          it works well for less than 1000 objects.
        </p>

        <p>
          This can be achieved with a{" "}
          <InlineCode code="for" meta="javascript" /> loop on each{" "}
          <InlineCode code="update" meta="javascript" /> of{" "}
  ...
  update(state, time, updateId) {
    ...
    return new Ball({
      ...
      position: this.position.add(this.velocity),
        ...
        <p>
          Because we are using a <InlineCode code="Vector" meta="javascript" />{" "}
          to track the position of the ball, we can measure the distance between
          two objects using the{" "}
...
runAnimation(time => {
  ...
  display.sync(state);
        ...
        <p>
          ...
          found it to be a fun animation without getting too complex with
          gravity and friction. But if that's what you're after, give it a shot.
        </p>
        ...
        <p>
          Mathematically this can be defined as the following. Keep in mind
          there is an alternate formula that uses angles, but because we are
          using vectors, this is an easier formula to implement.
        </p>
        ...
        <p>
          ...
          representation of its mass. Assuming a constant density, we can use
          the spherical area of the circle as our mass.
        </p>
  ...
  get sphereArea() {
    return 4 * Math.PI * this.radius ** 2;
        ...
        <p>
          ...
          in JavaScript. It's not a nice formula, but it is compact and
          accurate.
        </p>
    ...
    .subtract(
      ...
      .multiply(
        ...
        <p>
          ...
          <InlineCode code="Ball" meta="javascript" />. However, there is
          another issue we need to resolve. We can't update velocities one at a
          time because the current velocity of both actors is necessary to
          determine their new velocities. And if we update both actor's velocity
          now, the velocity will be updated twice because every actor checks
          every actor.
        </p>

        <p>
          So we need a way to update both actors together but only update them
          once. There is likely a better way, but what I came up with is
          creating an ID for each collision and keeping track of those IDs in an
          array so we can skip them in the current update. The collision ID is
          composed of the object ID and update ID.
        </p>

        <p>
          We already added an update ID to{" "}
          <InlineCode code="State" meta="javascript" /> so let's add an object
          ID to <InlineCode code="Ball" meta="javascript" /> as well as a
          collisions array. These are the pieces we need to calculate collisions
          in <InlineCode code="Ball.update" meta="javascript" /> so let's update
          that as well.
        </p>
...
runAnimation(time => {
  ...
  display.sync(state);
        ...
        <p>
          The first issue of wall collisions is relatively simple. We just need
          to give the position update an upper and lower bound based on the
          canvas walls.
        </p>
        ...
        <p>
          ...
          update that normally results in a frantic spiraling. It's an issue
          that only arises in very specific conditions, but becomes quite common
          with 10+ balls in the frame.
        </p>
        ...
        <p>
          ...
          in the next frame and update the current frame as if they had
          collided. This way the objects never get a chance to overlap. However,
          they never get a chance to collide either. Thankfully, the effect is
          barely noticeable.
        </p>

        <p>
          In pursuit of a better solution, I looked into calculating the amount
          of overlap and removing that distance from the current position before
          adding the new velocity. And it worked in a small environment, but
          once again issue arose when there were 10+ balls in the simulation.
          I'm not sure if this is due to float precision, or balls backing up
          into another.
        </p>

        <p>
          Wall squeezes occur when a ball hits the wall and another ball at the
          same time. I don't have a perfect solution for this. If wall
          collisions are calculated first, velocity is lost. If they are
          calculated after, velocity is gained. This is due to the bounding
          condition we created for wall collisions where the position is stopped
          exactly when the edge touches the wall. However, if I remove this
          bounding condition, objects can get stuck against the wall.
        </p>
        ...
        <p>
          Finally, because we are keeping track of each collision in each ball,
          it can quickly overwhelm memory. An easy fix is to trim down the size
          of the array at a certain limit. I chose 10 as the limit because I
          have a hard time thinking of a situation where a ball will
          simultaneously collide with 10 other balls. However, this could be
...
class Ball {
  ...
  ...
        ...
        <p>
          Now stickiness can only occur if two objects spawn on top of each
          other. Which can happen using a random generator like I did for the
...
runAnimation(time => {
  ...
  display.sync(state);
        ...
        <p>
          ...
          . But to wrap up the article, here is the script I used to generate
          the intro animation in this article.
        </p>
...
collidingBalls({
  ...
  parent: document.getElementById('hero'),
        ...
        <p>
          This was a lot of fun. But it's not perfect. There are still issues
          with imperfect collisions (because the balls do not touch), wall
          squeezes, and balls that are spawned on top of each other. If you come
          up with a solution for any of these things, please let me know!
        </p>
      </div>
    </>
  );
